function [sift_arr, grid_x, grid_y] = grid_sift_on_edge_map(I, num_bins, sigma)
...
patch_size = sum(size(I))/2;
grid_spacing = patch_size/2;
...
weight_x = gausswin(patch_size/2, sigma);
for a = 1:num_angles
...
if ~isempty( find( isnan(sift_arr) ) )
    keyboard;
end
