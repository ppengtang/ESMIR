...
void HoG(double *pixels, double *params, int *img_size, double *dth_des, unsigned int grayscale){
    ...
    double cwidth_x     =  img_width / params[2];
    double cwidth_y     =  img_height / params[2];
	
	// std::cout << cwidth_x << "\t" cwidth_y << std::endl; 
    int block_size    = (int) params[2];
    int orient        = (int) params[3];
    double clip_val   = params[4];
    
    int hist1= 2+ceil(-0.5 + img_height/cwidth_y);
    int hist2= 2+ceil(-0.5 + img_width/cwidth_x);
    ...
    for(unsigned int y=0; y<img_height; y++) {
        for(unsigned int x=0; x<img_width; x++) {
            ...
            bin2 = bin1 + 1;
            x1   = (int)floor(0.5+ x/cwidth_x);
            x2   = x1+1;
            y1   = (int)floor(0.5+ y/cwidth_y);
            y2   = y1 + 1;
            
            Xc = (x1+1-1.5)*cwidth_x + 0.5;
            Yc = (y1+1-1.5)*cwidth_y + 0.5;
            ...
            h[y1][x1][bin1]= h[y1][x1][bin1]+grad_mag*(1-((x+1-Xc)/cwidth_x))*(1-((y+1-Yc)/cwidth_y))*(1-((grad_or-Oc)/bin_size));
            h[y1][x1][bin2]= h[y1][x1][bin2]+grad_mag*(1-((x+1-Xc)/cwidth_x))*(1-((y+1-Yc)/cwidth_y))*(((grad_or-Oc)/bin_size));
            h[y2][x1][bin1]= h[y2][x1][bin1]+grad_mag*(1-((x+1-Xc)/cwidth_x))*(((y+1-Yc)/cwidth_y))*(1-((grad_or-Oc)/bin_size));
            h[y2][x1][bin2]= h[y2][x1][bin2]+grad_mag*(1-((x+1-Xc)/cwidth_x))*(((y+1-Yc)/cwidth_y))*(((grad_or-Oc)/bin_size));
            h[y1][x2][bin1]= h[y1][x2][bin1]+grad_mag*(((x+1-Xc)/cwidth_x))*(1-((y+1-Yc)/cwidth_y))*(1-((grad_or-Oc)/bin_size));
            h[y1][x2][bin2]= h[y1][x2][bin2]+grad_mag*(((x+1-Xc)/cwidth_x))*(1-((y+1-Yc)/cwidth_y))*(((grad_or-Oc)/bin_size));
            h[y2][x2][bin1]= h[y2][x2][bin1]+grad_mag*(((x+1-Xc)/cwidth_x))*(((y+1-Yc)/cwidth_y))*(1-((grad_or-Oc)/bin_size));
            h[y2][x2][bin2]= h[y2][x2][bin2]+grad_mag*(((x+1-Xc)/cwidth_x))*(((y+1-Yc)/cwidth_y))*(((grad_or-Oc)/bin_size));
        }
    }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    img_size[0] = (int) mxGetM(prhs[0]);
    img_size[1]  = (int) mxGetN(prhs[0]);
    ...
    nb_bins       = (mwSize) params[0];    
    block_size    = (mwSize) params[2];     
