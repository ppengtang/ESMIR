...
int read_problem_single(const mxArray *label_vec, const mxArray *instance_mat)
{
	long i, j, k;
	mwIndex *ir, *jc;
	long elements, max_index, num_samples, label_vector_row_num;
	...
	// suppose the matrix is in row format
	/*
	if(col_format_flag)
	...
	// figure out the non-zero values in the training data
	num_samples = 0;
	for (i=0; i<prob.l; i++)
	{
		for (k=0; k<max_index; k++)
		{
			if ( fabs(samples[i+k*prob.l]) > 1e-6 )
			{
				++num_samples;
			}
		}
	...
	mexPrintf("max_index %d\n", max_index);
	mexPrintf("prob.l %d\n", prob.l);
	mexPrintf("Memory has been alloced\n");
	*/
	...
	for(i=0;i<prob.l;i++)
	{
		...
		for(k=low;k<high;k++)
		{
			x_space[j].index = (int) ir[k]+1;		// should be the number of dimension
		...
		for (k=0; k<max_index; k++)
		{
			...
			{
				x_space[j].index = k+1;
				x_space[j].value = samples[i+k*prob.l];
				++j;
			}
		}
		...
		x_space[j++].index = -1;					// for each training example, it stop by a zeros value and a -1 index
	}
		...
		int nrhs, const mxArray *prhs[] )
{
	...
	if(nrhs > 1 && nrhs < 5)
	{
		...
		if(mxIsSingle(prhs[1]))
			err = read_problem_single(prhs[0], prhs[1]);
		else
		{
			mexPrintf("Training_instance_matrix must be single\n");
			destroy_param(&param);
			fake_answer(plhs);
			return;
		}
