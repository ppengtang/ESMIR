...
database.path = cell(1, 100000000); % contain the pathes for each image of each class
database.nclass = 0;
count = 0;
...
for ii = 1:length(subfolders),
    ...
    if ~strcmp(subname, '.') && ~strcmp(subname, '..'),
        ...
        for jj = 1:c_num,
            ...
            count = count + 1;
            database.path{count} = c_path;
        end;    
    end;
end;

database.path( count+1:end ) = [];

fprintf('There are %d files in %s.\n', length(database.path), rt_data_dir);
